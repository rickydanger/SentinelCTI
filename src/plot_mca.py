...
def prepare_data(mca_telemetry_json):
    ...
    for r in mca_telemetry_json:
        ...
        tool = r.get("related_tool", "Unmapped")

        entity_tactic[(e, tac)] += 1
        tactic_tech[(tac, t)] += 1
        tech_channel[(t, c)] += 1
        channel_artifact[(c, a)] += 1
        artifact_tool[(a, tool)] += 1
    ...
    return {
        ...
        "tool_names": [n for n, _ in tool_total.most_common()],
...
def get_sankey(mca_telemetry_json):
    ...
    for (a, tool), cnt in data["artifact_tool"].items():
        source.append(idx[a])
        target.append(idx[tool])
        value.append(cnt)
    ...
    fig.update_layout(
        title_text=(
            ...
            "<br>Entity → Tactic → Technique → Log Source → Related Artifact → Tool"
...
def get_table(mca_telemetry_json):
    ...
    fig = go.Figure(data=[go.Table(
        header=dict(
            values=[
                "Entity", "Tactic", "Technique", "Technique Count",
                "Log Source", "Channel Count", "Related Artifact", "Tool",
            ],
            fill_color="#1E88E5",
        ...
        cells=dict(
            values=[
                ...
                [row["related_tool"] for row in table_data],
            ],
            fill_color="#F5F5F5",
            align="left",
            font=dict(size=12)
